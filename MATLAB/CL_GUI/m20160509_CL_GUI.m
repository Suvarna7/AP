...
myaddress = 'artificialpancreaschicago@gmail.com';
mypassword = 'controllab34';
...
sendmail({'ihajizad@hawk.iit.edu','jfeng12@hawk.iit.edu'},...
...
%     props.setProperty('mail.smtp.socketFactory.port','465');
%     sendmail({'ihajizad@hawk.iit.edu','jfeng12@hawk.iit.edu'},...
%         'Data Backup (Error in Sensor error detection and functional redundancy with nosie generator)','An error occured during the current experiment and this email is sent to you automatically.',...
%         {'Error_commend_window.txt','prevdata.mat'});
%     delete('Error_commend_window.txt')
...
try
    ...
    %%%%%%% Must Be Defined
    if ttt_time(4)<=12.5
        basal_insulin(kj,1)=1;
    elseif ttt_time(4)>12.5 && ttt_time(4)<=16
        basal_insulin(kj,1)=1.1;
    elseif ttt_time(4)>16 && ttt_time(4)<=24
        basal_insulin(kj,1)=1;
    else
        basal_insulin(kj,1)=1.1;
    end
...
try
    ...
    if fault(kj,1)>0
        ...
        text(4,0.1,'Directional Change is Not Symmetric','rotation',90)
        text(5,0.1,'Peak or Flatness at the Directional Change','rotation',90)
        ylim([0 1])
    else
        axes(handles.gui_real_monitoring_figure);
        bar(fault_reason(kj,:)/max(fault_reason(kj,:)),'FaceColor','red','EdgeColor','red','LineWidth',1.5)
...
if hypo_alarm(kj,1)==1 && hypo_phase(kj,1)==5
...
elseif hypo_alarm(kj,1)==1 && hypo_phase(kj,1)==4
    ...
    set(handles.gui_hypo_phase0,'String',' ')
    basal_insulin(kj,1)=0;
elseif hypo_alarm(kj,1)==1 && hypo_phase(kj,1)==3
    set(handles.gui_hypo_phase5,'String',' ')
    set(handles.gui_hypo_phase4,'String',' ')
    ...
    set(handles.gui_hypo_phase0,'String',' ')
    basal_insulin(kj,1)=0;
elseif hypo_alarm(kj,1)==1 && hypo_phase(kj,1)==2
    set(handles.gui_hypo_phase5,'String',' ')
    set(handles.gui_hypo_phase4,'String',' ')
    set(handles.gui_hypo_phase3,'String',' ')
    ...
    set(handles.gui_hypo_phase0,'String',' ')
    basal_insulin(kj,1)=0;
elseif hypo_alarm(kj,1)==1 && hypo_phase(kj,1)==1
    set(handles.gui_hypo_phase5,'String',' ')
    set(handles.gui_hypo_phase4,'String',' ')
    set(handles.gui_hypo_phase3,'String',' ')
    set(handles.gui_hypo_phase2,'String',' ')
...
mdata1=cell(1,15);
mdata1(1,[1,2,3,4,5,6,8])=[{datestr(clock)},gs(kj,1),bolus_insulin(1,kj),basal_insulin(kj),hypo_alarm(kj,1),double(~isequal(meal_bolus_amount(kj,1)*meal_detection(kj,1),0)),batch_CL(kj,1)];
mdata=[mdata1;mdata];
...
if etime(clock,auto_backup_time)>3600
    ...
    props.setProperty('mail.smtp.socketFactory.port','465');
    sendmail({'ihajizad@hawk.iit.edu','jfeng12@hawk.iit.edu'},...
        'Data Backup (Auto Backup)','An error occured during the current experiment and this email is sent to you automatically.',...
        {'prevdata.mat','temp_prevdata.mat'});
end
set(handles.gui_insulin_verification,'Visible','on')
if flag_data_1==1 && flag_data_2==1 && flag_data_3==1 && flag_data_4==1 && flag_data_5==1 && flag_data_6==1 && flag_data_7==1 && flag_data_8==1
...
m20150711_load_global_variables
%%
gs_in=136;%must be defined before the experiment glocuse
ee_in=4.3;%must be defined before the experiment
gsr_in=0.0088;%must be defined before the experiment
sleep_in=0;%must be defined before the experiment
phys_act_in=0;%must be defined before the experiment
body_weight=60;%must be defined before the experiment kg
%%
gs=gs_in*ones(20,1);
%%
