...
j=1;
 thita_m(j,:)=thita_m_offline
    ...
    for i=1:N
        ...
        aradeger1=(x(:,i)-x_q)
        aradeger2=(x(:,i)-x_q)'
        aradeger3=(x(:,i)-x_q)'*thita
        aradeger4=(x(:,i)-x_q)'*thita*(x(:,i)-x_q)
        aradeger5= i
    ...
    %%%%%%%%%%%%%%
    aradeger7=sum(w)
    aradeger8=w*regression_coefficient_one'
    mean_rc=w*regression_coefficient_one'/sum(w)
    aradeger9=mean_rc'*ones(1,N)
    aradeger10=(regression_coefficient_one-mean_rc'*ones(1,N))'
    aradeger25=((regression_coefficient_one-mean_rc'*ones(1,N))').^2
    aradeger11=w*((regression_coefficient_one-mean_rc'*ones(1,N))').^2
    Var_weighted=w*((regression_coefficient_one-mean_rc'*ones(1,N))').^2/sum(w)
    aradeger12=Var_weighted.^alpha
    aradeger26=thita_m(j,:)
    aradeger13=Var_weighted.^alpha+thita_m(j,:)
    thita_m(j+1,:)=(Var_weighted.^alpha+thita_m(j,:))/2
    for mm=1:M
    sigma(mm)=abs((thita_m(j+1,mm)-thita_m(j,mm))/thita_m(j,mm))
    mm
    end
    aradegercikis=sigma2*ones(1,M)
    aradegercikis2=sigma<sigma2*ones(1,M)
    aradegercikis3=sum(sigma<sigma2*ones(1,M))
