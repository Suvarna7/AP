...
basal_insulin
bolus_insulin
ins_every5=basal_insulin(1,:)+bolus_insulin'*12%
[a,b]=size(CGM)
[c,d]=size(ins_every5)
sum_score=[]
sum_type=[]
...
if length(CGM)<22
    ...
    elseif noisel(kj-1)>1
        gb(kj)=gb(kj)+addsignal(5-noisel(kj-1))'
        noisel(kj)=noisel(kj-1)-1
        if type==6
            gb(kj)=gb(kj-1)
        end
    elseif noisel(kj-1)==1
    ...
    if kj>21
      ...
      %  fonksiyonagirmeden=kj
        ...
        w
        AA
        aracarpanz= x_e(:,end-1)
        aracarpanzz= x_e(:,end)
        fonksyindandonucne=kj
        adurumu=AA(:,:,kj-1)
        aracarpanxx=AA(:,:,kj-1)*x_e(:,end-1)
        aracarpanxxy=x_e(:,end)
    ...
    if kj>23
        Girdi=-99
        g_pls(kj)=y_pkal(1,end)*std_gb+mean_gb
         result1=gb(kj)
         result2=gb(kj-1)
    ...
    if kj>M_pls
        ...
        aras=gb_plss(kj-M_pls+1:kj)'
        inss
        inss=[inss, [0 0 0 0 0]]  %% Just for trying
        inss(kj-M_pls:kj-1)'
        aras1=inss(kj-M_pls:kj-1)'
        aras2=CGM_change_pls(kj-M_pls+1:kj)'
        %aras3=x_samp(:,kj)
        x_samp(:,kj)=[gb_plss(kj-M_pls+1:kj)';
        inss(kj-M_pls:kj-1)';
        CGM_change_pls(kj-M_pls+1:kj)']
        mert=x_samp
