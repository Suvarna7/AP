...
[meal_states(:,kj),meal_covariance(:,:,kj)]=m20141215_ukf_meal(gs(kj,1),meal_states(:,kj),meal_covariance(:,:,kj),R_meal,Q_p_meal,meal_g_basal(kj,1));
meal_states1=meal_states(:,kj)
gs1=gs(kj,1)
meal_detection1=meal_detection(kj-1,1)
meal_detection_time1=meal_detection_time(kj-1,1)
correction_detection_time1=correction_detection_time(kj-1,1)
correction_limit1=correction_limit(kj-1,1)
correction_bolus_amount1=correction_bolus_amount(kj-1,1)
body_weight
sleep1=sleep(kj,1)
phys_act1=phys_act(kj,1)
meal_gpc_mu1=meal_gpc_mu(kj,1)
...
meal_bolus_amount=meal_bolus_amount(kj,1)
correction_detection=correction_detection(kj,1)
meal_detection=meal_detection(kj,1)
meal_detection_time=meal_detection_time(kj,1)
correction_detection_time=correction_detection_time(kj,1)
correction_bolus_amount=correction_bolus_amount(kj,1)
