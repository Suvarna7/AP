...
[I_track]=controller_assessment_index_071215_JF(maximum_insulin, L,reference_glucose, insulin_sensitivity_factor,kj,gs,basal_insulin,body_weight,iction);
sensor_error=0;
flag_constrains=0;
...
if kj>25
    ...
    compensate_ins(kj)=insulin_error_feedback*(bolus_insulin(end)*12+basal_insulin(1,end));
    paralleled=1;
    [phi,phi_ee,phi_gsr,...
        ...
        reference_glucose(:,1:kj-1),insulin_sensitivity_constant(:,1:kj-1),basal_insulin(:,1:kj-1),IOB_prediction(:,1:kj-1),...
        maximum_insulin(:,1:kj-1),total_daily_unit(:,1:kj-1),insulin_sensitivity_factor(:,1:kj-1),body_weight,meal_gpc_mu,bolus_insulin(1:kj),...
    ...
    if isnan(error_summation(2,end)) &&  isnan(error_summation(3,end)) &&  isnan(error_summation(4,end)) && sensor_error(kj)==0 
      insulin_CPA(:,kj)=basal_insulin(:,kj);
    else 
