...
% Last Modified by GUIDE v2.5 24-Feb-2016 16:50:56
...
try gs=[gs;170];%m20150711_get_CGM_value(gs);
    kj=length(gs);
catch theErrorInfo  %sends an error email to Kamuran and Iman (Add your email if you want to receive data). The email includes error message, backup data and a screenshot of command window.
    diary('Error_commend_window.txt')
    disp(theErrorInfo.identifier)
...
%         {'Error_commend_window.txt','prevdata.mat'});
    delete('Error_commend_window.txt')
    set(handles.text12,'String','An error occured while CGM retuning with noise generator! Data is saved succesfully and sent to Kamuran and Iman. Please run the algorithm again.')
    set(handles.text12,'Visible','on')
    set(handles.pushbutton17,'Visible','on')
end
...
%         {'Error_commend_window.txt','prevdata.mat'});
    delete('Error_commend_window.txt')
    set(handles.text12,'String','An error occured while CGM retuning without noise generator! Data is saved succesfully and sent to Kamuran and Iman. Please run the algorithm again.')
    set(handles.text12,'Visible','on')
    set(handles.pushbutton17,'Visible','on')
end
%% Get armband data
...
%% Calculate IOB
try IOB_total(kj,1)=m20150711_calculate_IOB(bolus_insulin,basal_insulin(1,:));
catch theErrorInfo  %sends an error email to Kamuran and Iman (Add your email if you want to receive data). The email includes error message, backup data and a screenshot of command window.
    save temp_prevdata.mat
    diary('Error_commend_window.txt')
    disp(theErrorInfo.identifier)
