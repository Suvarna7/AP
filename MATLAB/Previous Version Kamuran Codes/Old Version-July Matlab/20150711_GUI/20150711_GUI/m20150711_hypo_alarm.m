...
if phys_act(kj,1)==1 %Carb suggestions during exercise
    [hypo_alarm(kj,1),carb_amount(kj,1),carb_type(kj,1),hypo_phase(kj,1),hypo_phase_old(kj,1),repeated_immediate_alarm(kj,1)]=m20150510_hypo_algorithm_exercise(gs(kj,1),g_prediction(5,kj),hypo_threshold(kj,1),hypo_phase_old(kj-1,1),hypo_slope_degree(kj,1));
elseif sleep(kj,1)==1 %Carb suggestions during sleep
    [hypo_alarm(kj,1),carb_amount(kj,1),carb_type(kj,1),hypo_phase(kj,1),hypo_phase_old(kj,1),repeated_immediate_alarm(kj,1)]=m20150510_hypo_algorithm_sleep(gs(kj,1),g_prediction(5,kj),hypo_threshold(kj,1),hypo_phase_old(kj-1,1),hypo_slope_degree(kj,1));
else %Carb suggestions during day
    [hypo_alarm(kj,1),carb_amount(kj,1),carb_type(kj,1),hypo_phase(kj,1),hypo_phase_old(kj,1),repeated_immediate_alarm(kj,1)]=m20150510_hypo_algorithm(gs(kj,1),g_prediction(5,kj),hypo_threshold(kj,1),hypo_phase_old(kj-1,1),hypo_slope_degree(kj,1));
end