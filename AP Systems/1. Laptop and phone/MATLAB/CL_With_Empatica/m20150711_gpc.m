...
phi(:,kj)=[-flipud(gs(kj-na:kj-1,1));flipud((bolus_insulin(1,kj-nb1-N1:kj-1-N1))');flipud(ee(kj-nb2+1-d2:kj-d2,1));flipud(gsr(kj-nb2+1-d3:kj-d3,1));flipud(armax_err(kj-nc:kj-1,1))];
...
ee_prediction(:,kj)=(M_ee(:,:,kj)*(A_state_ee(:,:,kj)-K_state_ee(:,:,kj)*C_state_ee(:,:,kj))*X_state_ee(:,kj))+(M_ee(:,:,kj)*K_state_ee(:,:,kj)*(ee(kj,1)));ee_prediction(ee_prediction(:,kj)<0,kj)=0;
gsr_prediction(:,kj)=(M_gsr(:,:,kj)*(A_state_gsr(:,:,kj)-K_state_gsr(:,:,kj)*C_state_gsr(:,:,kj))*X_state_gsr(:,kj))+(M_gsr(:,:,kj)*K_state_gsr(:,:,kj)*(gsr(kj,1)));gsr_prediction(gsr_prediction(:,kj)<0,kj)=0;
g_prediction(:,kj)=(M(:,:,kj)*(A_state(:,:,kj)-K_state(:,:,kj)*C_state(:,:,kj))*X_state(:,kj))+(M(:,:,kj)*K_state(:,:,kj)*(gs(kj,1)))+L_ee(:,:,kj)*[ee(kj-d2+1:kj,1);ee_prediction(1:end-d2,kj)]+L_gsr(:,:,kj)*[gsr(kj-d2+1:kj,1);gsr_prediction(1:end-d2,kj)];g_prediction(g_prediction(:,kj)<0,kj)=0;
if gs(kj)-g_prediction(1,kj)<=0
    for kk1=2:length(g_prediction(:,kj))
        if g_prediction(kk1-1,kj)-g_prediction(kk1,kj)>=0
            g_prediction(kk1,kj)=g_prediction(kk1-1,kj);
        end
    end
else
    for kk1=2:length(g_prediction(:,kj))
...
[bolus_insulin(:,kj),IOB_prediction(:,kj),maximum_insulin(:,kj),total_daily_unit(:,kj),insulin_sensitivity_factor(:,kj)]=controller_ins(g_prediction(:,kj),L(:,:,kj),bolus_insulin_meal,bolus_insulin,basal_insulin,0,reference_glucose(:,kj),Nu,st,body_weight,insulin_sensitivity_constant(:,kj),flag_constrains);
