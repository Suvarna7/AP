...
%% Correction bolus
if kj-meal_detection_time>=3/3 && x_p(3)>=correction_limit && kj-correction_time>=3/3 && meal_detection==1 && y>100 && sleep==0 && phys_act==0
    correction_detection=1;
    correction_limit=correction_limit+1/3;
    ISC=y/rr(1);
    TDD=ISC*body_weight;
    ISF=1800/TDD;
    correction_bolus=min(6,(y-rr(1))/ISF);
    correction_time=kj;
elseif kj-meal_detection_time>=3/3 && x_p(3)>=correction_limit && kj-correction_time>=3/3 && meal_detection==1 && y>140 && sleep==1
    correction_detection=1;
    correction_limit=correction_limit+1/3;
    ISC=y/rr(1);
    TDD=ISC*body_weight;
    ...
    correction_bolus=min(6,(y-rr(1))/ISF);
    correction_time=kj;
elseif kj-meal_detection_time>=3/3 && x_p(3)>=correction_limit && kj-correction_time>=3/3 && meal_detection==1 && y>300 && phys_act==1
    correction_detection=1;
    correction_limit=correction_limit+1/3;
    ISC=y/rr(1);
    TDD=ISC*body_weight;
