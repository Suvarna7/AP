...
public class SSM_param {
    ...
    public void initSSM_param(Tvector basal, Tvector CR, Tvector CF, double TDI, double BW) {
    	...
    	filter.alpha = 0.5;
    	filter.width=45;
    	//TODO filter.bolus_max=6;
    	filter.bolus_max=200;
    	// model
    	...
    	// SSM_brakes
    	brakes.predH = 30;
    	brakes.eA = new double [][] {
    			{26.506772331041630, -1.370936191391605e+04, -0.011771763925024, -0.080537390356092, -0.406493947513403, 0, 0.008518384470767, 0.015596111698848},
    			{0, 6.179100432059156, 1.340407744180351e-05, 6.545748834768742e-05, 1.968661709109575e-04, 0, 0, 0},
    			{0, 0, 22.657712170113810, 0, 0, 0, 0, 0},
    			{0, 0, 6.008882974261290, 22.657712170113810, 0, 0, 0, 0},
    			{0, 0, 0.406173301880580, 1.685233050588779, 4.718675515531188, 0, 0, 0},
    			{17.424047571422168, -8.436547592065199e+03, -0.004514005481199, -0.036794501325990, -0.231350734976448, 10.757333205713707, 0.004004453760185, 0.008271581209935},
    			{0, 0, 0, 0, 0, 0, 10.299677710160346, 0},
    			{0, 0, 0, 0, 0, 0, 17.388833653769910, 25.705042713119482}
    	};
    	// SSM_brakes_param
    	brakes_param.alpha = 0.5;
    	
    	double mean_cf = mean_value(CF);
    	TDI = check_tdi(TDI);
    	if (mean_cf < 0.0 || TDI < 0.0) { 			// Check for empty CF profile or invalid TDI value
    		brakes_param.k = -1.0;
    		isValid = false;
    	}
    	else {
...
//    	brakes_param.risk.a_ex = 0.2936;					// PKH, CHK - 1/18/12
    ...
    public double check_tdi(double TDI) {
    	double result;
    	//TODO: Use SysMan constants to check TDI value instead of these hard-coded ones
    	if (Double.isNaN(TDI) || Double.isInfinite(TDI) || TDI < 10.0 || TDI > 100.0) {
    		result = -1.0;
    	}
    	else {
    		result = TDI;
