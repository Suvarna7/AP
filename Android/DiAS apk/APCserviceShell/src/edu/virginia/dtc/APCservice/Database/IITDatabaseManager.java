...
public class IITDatabaseManager {
	 ...
	 * initDatabase ()
	 */
	private void initDatabase1(Context ctx){
		dbContext = ctx;
	...
	public  void updateSyncStatus(Context ctx, String table, String status_col, String status, String last_up){
		...
		updateSingleSample( db1, table,  status_col,  status,  last_up, SensorsManager.empatica_columnsTable[SensorsManager._TIME_INDEX]);
		db1.close();
	}
	
	/**
	 * Update one sample value for the given table in the corresponding database
	 * @param db open database
	 * @param table name of the table
	 * @param status_col  column to be updated
	 * @param status value to update
	 * @param last_up reference column: timestamp
	 */
	private void updateSingleSample(SQLiteDatabase db, String table, String status_col, String status, String last_up, String time_column){
		//Update query: update status, searching the right last_updated value
				String updateQuery = "UPDATE "+table +" SET "+status_col+" = '"+status+"' WHERE "+time_column+" = '"+ last_up+"'";
	...
	/**
	 * Update received sample and all previous
	 * (Accumulative ACK)
	 * @param ctx - context
	 * @param table - name of the table
	...
	public  int countRows(Context ctx, String table){
		int rows = 0;
		db=ctx.openOrCreateDatabase(databaseFile, SQLiteDatabase.OPEN_READWRITE, null);
