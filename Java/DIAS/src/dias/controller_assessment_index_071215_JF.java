...
public class controller_assessment_index_071215_JF {
    ...
    public Matrix controller_assessment_index_071215_JF() throws Exception {
        ...
        if (kj <= 21) {
            ...
            I_meinst = 0;
            I_error_rspeed = 0;
            I_uconstrain = new Matrix(TrackData.I_u_constrain.getRowDimension() + 1, 1);
            datamem = new Matrix(15, b);

            I_me = 0;
            I_u_contraint_data = 0;
            I_cost_function = 0;

            TrackData.I_track = Itrack;
            TrackData.I_error_rspeed = I_error_rspeed;
            
            TrackData.data_mem = datamem;
            TrackData.I_me_inst = I_meinst;

            for (int i = 0; i < TrackData.I_u_constrain.getRowDimension(); i++) {
                for (int j = 0; j < TrackData.I_u_constrain.getColumnDimension(); j++) {
                    TrackData.I_u_constrain.set(i, j, I_uconstrain.get(i, j));
                }
            }
        ...
        if (kj > 21) {
            ...
            I_uconstrain = new Matrix(TrackData.I_u_constrain.getRowDimension() + 1, TrackData.I_u_constrain.getColumnDimension());
            ...
            for (int i = 0; i < TrackData.I_u_constrain.getRowDimension(); i++) {
                for (int j = 0; j < TrackData.I_u_constrain.getColumnDimension(); j++) {
                    I_uconstrain.set(i, j, TrackData.I_u_constrain.get(i, j));
                }
            }
        ...
        if (kj > start_time) {
            ...
            if (Math.abs(ins_f.get(0, kj) - umaxx.get(0, 0)) < 0.0001) {
                I_uconstrain.set(TrackData.I_u_constrain.getRowDimension(), 0, 1);
            } else {
                I_uconstrain.set(TrackData.I_u_constrain.getRowDimension(), 0, 0);
            }
            ...
            if (kj < start_time + 5) {
                ...
                I_u_contraint_data = I_uconstrain.get(TrackData.I_u_constrain.getRowDimension() - 2, 0) + 
                                    I_uconstrain.get(TrackData.I_u_constrain.getRowDimension() - 1, 0) + 
                                    I_uconstrain.get(TrackData.I_u_constrain.getRowDimension(), 0);
            }
            ...
            //TODO CHANGED -   original          for (int i = 0; i < Itrack.getRowDimension(); i++) {
            for (int i = 0; i < Itrack.getRowDimension(); i++) 
                TrackData.data_mem.set(i, b - 1, Itrack.get(i, 0));
      
            TrackData.I_track = Itrack;
            TrackData.I_u_constrain = new Matrix(TrackData.I_u_constrain.getRowDimension() + 1, TrackData.I_u_constrain.getColumnDimension());
            ...
            b++;
            System.out.println("Create data_mem with: b "+ b);
            //Increase data_mem columsn by one, for next iteration of saving/loading
            TrackData.data_mem = createnewMatrix(15, b, TrackData.data_mem);
            System.out.println("data_mem size: " + TrackData.data_mem.getColumnDimension() +
                        " x "+ TrackData.data_mem.getRowDimension());
        }
